import os
import random

# 1. 경로 설정
current_dir = os.path.dirname(os.path.abspath(__file__))
base_dir = os.path.join(current_dir, "..", "Easy", "P04")

if not os.path.exists(base_dir):
    os.makedirs(base_dir, exist_ok=True)

TICK = "`" * 3

# ---------------------------------------------------------
# 2. 문제 설명 (problem.md)
# ---------------------------------------------------------
md_content = f"""# 하늘이의 최고의 가수 선발 (Best Singer)

## 문제 설명
오디션 프로그램의 진행자 **'하늘이'** 는 공정한 심사를 위해 가수들의 점수를 계산하는 프로그램을 만들려고 합니다.

한 명의 가수는 총 $N$명의 심사위원에게 점수를 받습니다. **'하늘이'** 는 점수의 편차를 줄이기 위해 다음 규칙을 적용합니다.

1. 전체 점수 중 **가장 높은 점수** 하나와 **가장 낮은 점수** 하나를 제외합니다.
2. 제외하고 남은 점수들의 **평균**을 구합니다.

가수가 받은 점수들이 주어질 때, **'하늘이'** 의 규칙에 따라 계산된 최종 평균 점수를 출력하는 프로그램을 작성하세요.

---

## 입력 형식 (Input Format)
* 첫 번째 줄에 심사위원의 수 $N$이 주어집니다. ($3 \le N \le 20$)
* 두 번째 줄에 $N$개의 점수가 공백으로 구분되어 주어집니다. (점수는 1 이상 100 이하의 정수)

## 출력 형식 (Output Format)
* 최댓값과 최솟값을 하나씩 제외한 나머지 점수들의 평균을 **소수점 둘째 자리**까지 출력합니다.

---

## 입출력 예시 (Sample I/O)

### 예시 1
**Input:**
{TICK}
5
10 20 30 40 50
{TICK}

**Output:**
{TICK}
30.00
{TICK}

> **설명:** 최댓값(50)과 최솟값(10)을 제외한 나머지 점수는 [20, 30, 40]입니다. 이들의 평균은 $(20+30+40) / 3 = 30.00$ 입니다.

### 예시 2
**Input:**
{TICK}
4
85 90 95 80
{TICK}

**Output:**
{TICK}
87.50
{TICK}

> **설명:** 최댓값(95)과 최솟값(80)을 제외한 나머지 점수는 [85, 90]입니다. 평균은 $(85+90) / 2 = 87.50$ 입니다.
"""

# ---------------------------------------------------------
# 3. 정답 코드 (solution.py) - 엔터 즉시 종료 버전
# ---------------------------------------------------------
py_solution = """import sys

def main():
    try:
        # 첫 번째 줄 N 입력
        line1 = input()
        if not line1: return
        n = int(line1.strip())
        
        # 두 번째 줄 점수 리스트 입력
        line2 = input()
        if not line2: return
        scores = list(map(int, line2.split()))
        
        # 최댓값과 최솟값 찾기
        max_val = max(scores)
        min_val = min(scores)
        
        # 전체 합에서 최댓값과 최솟값을 한 번씩 뺌
        total_sum = sum(scores) - max_val - min_val
        
        # 평균 계산 (N-2개로 나눔)
        avg = total_sum / (n - 2)
        
        # 소수점 둘째 자리까지 출력
        print(f"{avg:.2f}")
            
    except (EOFError, ValueError, ZeroDivisionError):
        pass

if __name__ == "__main__":
    main()
"""

# ---------------------------------------------------------
# 4. 파일 저장 및 테스트케이스 생성
# ---------------------------------------------------------
def save_file(path, content):
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)

save_file(os.path.join(base_dir, "problem.md"), md_content)
save_file(os.path.join(base_dir, "solution.py"), py_solution)

for i in range(1, 21):
    n = random.randint(3, 10)
    scores = [random.randint(10, 100) for _ in range(n)]
    
    # 정답 계산
    max_s = max(scores)
    min_s = min(scores)
    avg = (sum(scores) - max_s - min_s) / (n - 2)
    ans_str = f"{avg:.2f}"
    
    # 실제 줄바꿈을 사용하여 파일 저장
    file_path = os.path.join(base_dir, f"input_{i:02d}.txt")
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(f"{n}\n")  # 여기서 실제 엔터 처리가 됩니다.
        f.write(" ".join(map(str, scores)))
        
    save_file(os.path.join(base_dir, f"output_{i:02d}.txt"), ans_str)

print(f"✅ 'Easy/P04' 생성이 완료되었습니다.")